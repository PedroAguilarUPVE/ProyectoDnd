...
public class VentanaPadre extends JFrame {
	...
	private static String[] Carreras = { // este tendria que ser llamado desde una base de datos
			"Ing. Sistemas Computacionales", "Ing. Agrotecnolog�a", "Ing. Manejo de Recursos Naturales",
	...
	private class ManejadorBoton implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource().equals(btnLimpiar)) {
				...
				/*
				 * Inicializa una variable con el tipo de ventana hija es la que se va ha llamar
				 * se parametriza el costructor Super de la ventana hija para que resiva desde
				 * que frame se esta abriendo y la variable boleana habilita el envio de
				 * parametros
				 */
				Ventana = new VentanaHijas(VentanaPadre.this, true,Idioma);
				// Se accede a la variable Etiqueta de la ventana Hija y se le asigna el
				// Valor del CoboBox
				Ventana.lblEtiqueta.setText((String) JcBCarreras.getSelectedItem());
				// Se envia e1alor de la variable contador actual de la p�gina padre
				Ventana.ContadorHijo = Contador;
				// se visualiza la ventana Hija
				...
				lblContadorVisitas.setText(Integer.toString(Ventana.ContadorHijo));
